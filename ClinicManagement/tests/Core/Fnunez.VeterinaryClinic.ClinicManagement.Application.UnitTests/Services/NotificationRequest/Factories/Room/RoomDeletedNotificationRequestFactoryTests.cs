...
public class RoomDeletedNotificationRequestFactoryTests
{
    private readonly Guid _correlationId = Guid.NewGuid();
    private readonly RoomDeletedNotificationRequestFactory _factory;
    private readonly int _roomId = 1;
    private readonly string _roomName = "a";
    private readonly string _userId = Guid.NewGuid().ToString();

    public RoomDeletedNotificationRequestFactoryTests()
    {
    ...
    public void GetNotificationEvent_ReturnsNotificationEvent()
    {
        ...
        // Assert
        Assert.Equal(
            NotificationEvent.RoomDeleted.ToString(),
            notificationEvent
        );
    }
}